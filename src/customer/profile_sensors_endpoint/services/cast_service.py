...
class CastService(CastHotSpotQueries):
    ...
    async def get_cast_stats(self, customer_id: str, sensor : str) -> Any:
        try:
            ...
            async for conn1 in connections:
                connection_startDate = datetime.strptime(conn1['start'], '%Y-%m-%dT%H:%M:%S.%f')
                connection_endDate = datetime.strptime(conn1['end'], '%Y-%m-%dT%H:%M:%S.%f')
    ...
    async def get_cast_history(self, customer_id: str, sensor : str, skip: Optional[int] = 0, limit: Optional[int] = None):
